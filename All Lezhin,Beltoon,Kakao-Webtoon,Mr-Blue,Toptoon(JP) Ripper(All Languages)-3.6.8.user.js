...
(function() {
    ...
    const state = {
        urlToBlob: new Map(),
        urlToHash: new Map(),
        blobHookRegistry: new Map(), // Dedicated queue for Canvas-only sites (Mr. Blue)
        pageRegistry: new Map(),     // The master source of truth for the UI and Downloader
        seenHashes: new Set(),
        masterDomList: [],           // Persistent array for Global Sequence Splicing
    ...
    targetWindow.URL.createObjectURL = function(blob) {
        ...
        try {
            if (blob instanceof Blob) {
                // SKIP BYTES sized images (at least 1KB)
                if (blob.size < 1024) return url; 

                state.urlToBlob.set(url, blob);

                computeSHA256(blob).then(hash => {
                    state.urlToHash.set(url, hash); // Global Deduplication Core
                    
                    if (!state.seenHashes.has(hash)) {
                        state.seenHashes.add(hash);

                        // Populates the Canvas queue completely independent of the DOM parser
    ...
    function syncPagesWithDOM() {
        ...
        if (cuts.length > 0) {
            ...
            // Global Splicer Fallback: Gathers physical Top-to-Bottom order
            ...
            if (domImages.length > 0) {
                // Splicer Logic: Anchors new images relative to known overlaps, 
                // perfectly preserving virtual scrolling history while maintaining physical order.
                ...
                // Pure Canvas Fallback (Mr. Blue, Bomtoon Descramble)
                // If no images exist in DOM, strictly pull from the independent Hook queue
